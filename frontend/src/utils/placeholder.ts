...
export function placeholderGradient(name: string, type: PlaceholderType): string {
  ...
  const middle = type === 'album' ? '#D4C5A0' : '#94A3B8';
  const middleOffset = type === 'album' ? '42%' : '48%';
  return `linear-gradient(${angle}deg, ${from} 0%, ${middle} ${middleOffset}, ${to} 100%)`;
}
...
export function placeholderImage(name: string, type: PlaceholderType): string {
  ...
  const middle = type === 'album' ? '#D4C5A0' : '#94A3B8';
  const middleOffset = type === 'album' ? '42%' : '48%';
  const sheenBreak = type === 'album' ? '38%' : '45%';
  const goldSheenOpacity = type === 'album' ? '0.28' : '0.18';
  const svg = `
    <svg xmlns="http://www.w3.org/2000/svg" width="600" height="600" viewBox="0 0 600 600">
      <defs>
        ...
        <linearGradient id="sheen" x1="0%" y1="0%" x2="100%" y2="100%">
          ...
          <stop offset="${sheenBreak}" stop-color="rgba(255,255,255,0)"/>
          <stop offset="100%" stop-color="rgba(200,168,98,${goldSheenOpacity})"/>
        </linearGradient>
      </defs>
